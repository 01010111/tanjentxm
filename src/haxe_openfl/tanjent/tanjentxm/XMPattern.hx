...
class XMPattern
{
	...
	public var patternData:Array <Int>;	
	 ...
	 */	
	public function new(numberOfRows:Int, numberOfChannels:Int) 
	{
		...
		this.patternData = new Array<Int>();
		for (i in 0 ... (numberOfChannels * numberOfRows * 5)) {
	 ...
	 */
	private inline function indexOfNote(channel:Int, row:Int):Int
	{
	 ...
	 * @param row the row number
	 */
	public function setNote(note:XMNote, channel:Int, row:Int)
	{
		var index:Int = indexOfNote(channel, row);
	 ...
	 * @param row the row number
	 */
	public function fillNote(note:XMNote, channel:Int, row:Int)
	{				
		var index:Int = indexOfNote(channel, row);
